    ...
    shiny::fluidPage(
      ...
      shiny::sidebarLayout(
        shiny::sidebarPanel(
          ...
          shiny::sliderInput("participation_rate_in_ss", "Percentage of informal workers who sign up for social security immediately",
                      min = 0, max = 100, value = 5, step = 1),
          shiny::sliderInput("participation_rate_in_ss_yearly_growth", "Additional sign up percentage per year",
                      min = 0, max = 100, value = 1, step = 0.5),
          shiny::radioButtons("give_subsidy_only_to_subgroup", label="Give subsidy only to a subgroup (e.g. self-employed) of all informal workers",
          ...
          shiny::numericInput("inflation_rate", "Inflation rate (%) - populated based on World Bank data",
                      min = -2, max = 50, value = 5, step = 1),
          shiny::numericInput("interest_rate", "Interest rate (%)",
